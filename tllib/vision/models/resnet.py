...
from torchvision.models import ResNet50_Weights  # Use pre-trained weights
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    # Ensure we select the correct ResNet variant
    if arch == "resnet18":
    ...
    if pretrained:
        ...
        # Only keep keys that match the model's layers
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and model_dict[k].shape == v.shape}

        model.load_state_dict(pretrained_dict, strict=False)  # Load without strict mode
