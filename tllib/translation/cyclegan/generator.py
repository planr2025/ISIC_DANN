...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        """Initialize the ResNet block with skip connections"""
        super(ResnetBlock, self).__init__()
        self.conv_block = self.build_conv_block(dim, padding_type, norm_layer, use_dropout, use_bias)

    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        """Construct a convolutional block with valid padding handling."""
        conv_block = []
        p = 1  # Default padding
        ...
        if padding_type == 'reflect':
            ...
            p = 1  # Ensure zero-padding consistency
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='zero'):
        """Construct a ResNet-based generator with flexible padding.
        ...
        # Change padding type to avoid errors
        if padding_type == 'reflect':
            model += [nn.ReflectionPad2d(3)]
        elif padding_type == 'replicate':
            model += [nn.ReplicationPad2d(3)]
        elif padding_type == 'zero':
            p = 3  # Standard zero padding
        else:
            raise NotImplementedError(f"Padding type [{padding_type}] is not implemented")

        model += [
            nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0 if padding_type in ['reflect', 'replicate'] else p, bias=use_bias),
            norm_layer(ngf),
            nn.ReLU(True)
        ]
        ...
        model += [
            nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0 if padding_type in ['reflect', 'replicate'] else p),
            nn.Tanh()
        ]
