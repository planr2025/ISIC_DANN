...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        # Dynamically compute the flattened feature dimension
        feature_dim = x.numel() // batch_size  # Total elements / batch size

        x = x.view(batch_size, feature_dim)  # Flatten dynamically

        if feature_dim != 2048:
            self.reduce_dim = nn.Linear(feature_dim, 2048).to(x.device)  # Move to correct device
            x = self.reduce_dim(x)  # Reduce to 2048-dim
...
class ClassifierHead(nn.Module):
    def __init__(self, bottleneck_dim, width,num_classes):
        super(ClassifierHead, self).__init__()
        self.layers = nn.Sequential(
            nn.Linear(bottleneck_dim, width),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(width, num_classes)  # Change num_classes to 1
        )
...
#     def __init__(self, backbone: nn.Module, bottleneck_dim, width,
#                  grl=None, finetune=True, pool_layer=None):
#         grl_layer = WarmStartGradientReverseLayer(alpha=1.0, lo=0.0, hi=0.1, max_iters=1000,
#                                                   auto_step=False) if grl is None else grl
       
#         pool_layer = pool_layer if pool_layer is not None else nn.Sequential(
#                 nn.AdaptiveAvgPool2d(output_size=(1, 1)),  
#                 nn.Flatten(start_dim=1)
#             )
#         bottleneck = Bottleneck()  # Ensure it returns (batch_size, bottleneck_dim)

#         # Define classifier heads ensuring output shape (batch_size, 1)
#         head = ClassifierHead(bottleneck_dim, width, num_classes=2)  
#         adv_head = ClassifierHead(bottleneck_dim, width, num_classes=2)  

#         super(ImageClassifier, self).__init__(backbone, 1, bottleneck, head, adv_head, grl_layer, finetune)

#     def forward(self, x):
#         """ Forward pass through backbone, bottleneck, and classifier heads """
#         features = self.backbone(x)  
#         # print(f"Backbone output shape: {features.shape}")  # Should be (batch_size, feature_dim)

#         features = self.bottleneck(features)  
#         # print(f"Bottleneck output shape: {features.shape}")  # Should be (batch_size, bottleneck_dim)

#         outputs = self.head(features)  # Main classifier head
#         outputs_adv = self.adv_head(features)  # Adversarial classifier head

#         # print(f"Main classifier output shape: {outputs.shape}")  # Should be (batch_size, 1)
#         # print(f"Adversarial output shape: {outputs_adv.shape}")  # Should be (batch_size, 1)
...
#         # ✅ Call parent class with the required arguments
#         super(ImageClassifier, self).__init__(backbone, 2, bottleneck, head, adv_head, grl_layer, finetune)

#         # 🔥 Ensure attributes are assigned properly
#         self.pool_layer = pool_layer  # Store pool layer as instance attribute

#     def forward(self, x):
#         """ Forward pass through backbone, pooling, bottleneck, and classifier heads """
#         features = self.backbone(x)  
#         features = self.pool_layer(features)  # ✅ Apply pooling
#         features = self.bottleneck(features)  

#         outputs = self.head(features)  
#         outputs_adv = self.adv_head(features)  
...
class ImageClassifier(GeneralModule):
    def __init__(self, backbone: nn.Module, bottleneck_dim, width,
        ...
        super(ImageClassifier, self).__init__(backbone, 2, None, None, None, grl_layer, finetune)
        # ✅ Define Pooling Layer inside a Sequential Model

        # ✅ Store pool layer as an instance attribute
        # self.pool_layer = pool_layer if pool_layer is not None else nn.Sequential(
        #     nn.AdaptiveAvgPool2d(output_size=(1, 1)),  
        #     nn.Flatten(start_dim=1)  
        # )
        self.pool_layer = nn.Identity()  # Identity layer
        self.bottleneck = Bottleneck()  # Ensure it outputs (batch_size, bottleneck_dim)

        self.features = nn.Sequential(
            backbone,   
            self.pool_layer,  # ✅ Use self.pool_layer
            self.bottleneck   
        )

        self.head = ClassifierHead(bottleneck_dim, width, num_classes=2)  
        self.adv_head = ClassifierHead(bottleneck_dim, width, num_classes=2) 
    ...
    def forward(self, x):
        ...
        # print("Backbone output shape:", features.shape)  # Debugging

        features = self.pool_layer(features)  # Pooling step
        # print("After pooling shape:", features.shape)  # Should be (B, C)

        features = self.bottleneck(features)  # Bottleneck step
        outputs = self.head(features)
        outputs_adv = self.adv_head(features)
        return outputs, outputs_adv
