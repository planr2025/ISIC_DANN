...
#     def __init__(self, in_feature):
#         super(ANet, self).__init__()
#         self.fc1 = nn.Linear(in_feature, 128)  # Hidden layer
...
#     test error of a classifier trained to discriminate the source from the target.
#     """
#     # Ensure labels are float tensors for BCE loss
#     source_label = torch.ones((source_feature.shape[0], 1), dtype=torch.float32)
#     target_label = torch.zeros((target_feature.shape[0], 1), dtype=torch.float32)
...
#             anet.zero_grad()
#             y = anet(x)
#             loss = F.binary_cross_entropy(torch.sigmoid(y), label)  # Ensure sigmoid applied
#             loss.backward()
#             optimizer.step()
...
#                 label = label.to(device)
#                 y = anet(x)
#                 acc = binary_accuracy(torch.sigmoid(y), label)  # Apply sigmoid before thresholding
#                 meter.update(acc, x.shape[0])

#         error = 1 - meter.avg / 100
#         error = max(0.0, min(error, 0.5))  # Clip error to [0, 0.5]
#         a_distance = 2 * (1 - 2 * error)

#         if progress:
#             print(f"Epoch {epoch} Accuracy: {meter.avg:.2f}%  A-dist: {a_distance:.4f}")
